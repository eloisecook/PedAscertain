#' @title Single pedigree simulation
#' @description Simulates one pedigree.
...
#' the ascertainment requirements specified by the user, then it outputs only this pedigree.
...
pedigree <- function(aff, frailty, frail_var, dis_haz, unaff_death_haz,
                     aff_death_haz, a_start, a_end, f_start, f_end, GRR){
  # j relates to the FamID when wanting to simulate multiple families
  j = 1
  ped <- myPed(aff, frailty, frail_var, j, dis_haz, unaff_death_haz,
               aff_death_haz, a_start, a_end, f_start, f_end, GRR)
  # Calculate event age for each individual (disease onset or censoring)
  full <- full_c(ped, a_end)
  return(full)
}
